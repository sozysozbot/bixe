 ...
 */
export function getSinglyAnnotatedLine(full_text: string, source: Source, highlight_?: { beginIndex: number, endIndex: number, match: string }) {
        ...
        switch (tok.kind) {
            case "pmcp-word": {
                if (isEarthlingWord(tok.content)) {
                    single_line.append(getHoverableForEarthlingWord(maybe_highlighted,
                        tok.content
                    ));

                    const expected_sources = expectedSourcesForEarthlingWord(tok.content);
                    if (
                        expected_sources.includes(source)
...
export function tokenize(full_text: string): Token[] {
    const ans: Token[] = [];
